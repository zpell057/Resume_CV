...
document.getElementById('CoopProjMenu').addEventListener('click',function(){scrollTo('Hack1Title');})
...
document.getElementById('menuHackathons').addEventListener('click',function(){scrollTo('Hack1Title');})
...
const en={
    ...
    "distMeasureShortDesc":"Ultrasonic distance sensor interface in Verilog HDL",
    "distMeasureDesc":"To learn how to design logic circuits using Verilog HDL, I built a module to interface with an ultrasonic distance sensor (HC-SR04) and output the estimated distance between the module and the next object in front of it in hexadecimal.\nI then made another module to convert that distance to decimal (BCD) using the double dabble algorithm.\nFinally, I added modules to display the distance on seven segment displays and uploaded the design on an Altera Max II CPLD to test the accuracy of the system. ",
...
const fr={
    ...
    "distMeasureShortDesc":"Interface avec un capteur de distance ultrasonique avec des modules Verilog HDL",
    "distMeasureDesc":"Pour apprendre à concevoir des circuits logiques à l’aide de Verilog HDL, j’ai construit un module qui s’interface avec capteur de distance à ultrason (HC-SR04) et générer la distance estimée entre le module et l’objet placé devant lui.\nJ’ai ensuite créé un autre module pour convertir cette distance en décimal (DCB) en utilisant l’algorithme double dabble.\nEnfin, j’ai ajouté des modules pour afficher la distance sur des afficheurs sept segments et j’ai téléversé la conception sur un CPLD Altera Max II pour tester la précision du système.",
    ...
    "Hack1Desc":"Dans le cadre du hackathon uOttaHack 2025, j’ai collaboré avec mon ami développeur logiciel pour concevoir une solution matériel-logiciel destinée à aider les personnes malentendantes à ressentir la musique par des vibrations.\nAu cours du week-end, mon ami a développé la partie logicielle : une application web avec une interface utilisateur esthétique qui communique avec l’API de Spotify afin de proposer des recommandations musicales à l’utilisateur.\nDe mon côté, j’ai conçu la partie matérielle de la solution avec les modules ESP-32 fournis par Seeed Studio, un des commanditaires de l’événement. La solution utilisait un relais connecté à un circuit transistor BJT de commutation pour permettre au microcontrôleur de transmettre des vibrations à l’utilisateur par le cliquetis du relais. Le microcontrôleur exploitait le microphone intégré pour analyser le signal audio au moyen d’un algorithme de transformée de Fourier rapide (FFT). J’ai ensuite élaboré un algorithme personnalisé pour traduire les fréquences issues de l’analyse de Fourier en fréquences plus basses, adaptées au relais.\n",
